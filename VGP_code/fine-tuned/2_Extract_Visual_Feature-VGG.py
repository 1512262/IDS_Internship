...
def get_pretrained_model(type):
    ...
    pretrained = Model(inputs=vgg.input, outputs=vgg.get_layer(index=arr[type]).output)
    del vgg
    return pretrained
...
def main():
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.type)
    print('VGG-16 for finetune',args.type,'layers')
    print('Using GPU',args.type)
    pretrained = get_pretrained_model(args.type)

    img_dir='../data/flickr30k-images/'
    emb_dir='../data/old_VGP/vgg_visual_embedding/'+str(args.type)+'/'
